...
import { addPlayer, removePlayer, changePlayerColor, startGame, } from '../redux/actions';
export class InputHandler {
    ...
    handleClick(clientX, clientY) {
        ...
        for (const entry of this.currentLayout.playerEntries) {
            if (this.isPointInRect(x, y, entry.colorIconX, entry.colorIconY, entry.colorIconSize, entry.colorIconSize)) {
    ...
    handleColorPickerClick(x, y) {
        if (!this.currentLayout?.colorPicker)
            return;
        ...
        if (!this.isPointInRect(x, y, picker.x, picker.y, picker.width, picker.height)) {
            this.renderer.hideColorPicker();
            return;
        }
        ...
        for (let index = 0; index < PLAYER_COLORS.length; index++) {
            ...
            if (this.isPointInRect(x, y, swatchX - colorSize / 2, swatchY - colorSize / 2, colorSize, colorSize)) {
